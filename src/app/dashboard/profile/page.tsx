...
function page() {
  ...
  if (typeof window !== "undefined") {
    Helper.isLoginUser();
    activeUsername = localStorage.getItem("user");
  }
  const fileInput = useRef<HTMLInputElement>(null);
  let [activeUser, setActiveUser] = useState<any>([]);
  let [isActiveUser, setIsActiveUser] = useState<boolean>(false);
  let [isLoading, setLoading] = useState<boolean>(true);

  useEffect(() => {
    getActiveUser(username, type);
  }, []);
  ...
  const getActiveUser = async (username: any = null, type: any = null) => {
    ...
    try {
      ...
      if (res?.data?.length > 0 && res.status === 200) {
        ...
        isActiveUser = data.some(
          (o: any) => o.username === activeUsername && o.status === "active"
        );
